...
int main(int argc, char *argv[])
{
    ...
    while (reader.getLine(line, line_id))
    {
        ...
        cout << "\n\nBoyer-Moore-Horspool:\n===================================================\n\n";
        {
            ...
            unsigned int n = str.length(); // Length of the string
            unsigned int m = pattern.length(); // Length of the pattern
            unsigned int offset = 0; // Offset (position of the pattern)
            unsigned int i = m - 1; // Character to test in pattern
            unsigned int j = i + offset; // Character to test in string
